...
// ─── STORAGE ──────────────────────────────────────────────────
function loadDiario()     { try { const r=localStorage.getItem("diario-v3");    return r?JSON.parse(r):{}; }    catch(e){ return {}; } }
function saveDiario(d)    { try { localStorage.setItem("diario-v3", JSON.stringify(d)); } catch(e){} }
function loadCompliance() { try { const r=localStorage.getItem("compliance-v1"); return r?JSON.parse(r):{}; }  catch(e){ return {}; } }
function saveCompliance(d){ try { localStorage.setItem("compliance-v1", JSON.stringify(d)); } catch(e){} }
function loadConfig()     { try { const r=localStorage.getItem("config-v1");    return r?JSON.parse(r):{b3:DEFAULT_BANCA_B3,forex:DEFAULT_BANCA_FOREX}; } catch(e){ return {b3:DEFAULT_BANCA_B3,forex:DEFAULT_BANCA_FOREX}; } }
function saveConfig(d)    { try { localStorage.setItem("config-v1", JSON.stringify(d)); } catch(e){} }
...
function RuleCard({ rule, checked, onToggle }) {
  ...
  return (
    <div onClick={onToggle} style={{background:checked?(rule.personal?"rgba(245,158,11,0.07)":"rgba(0,212,170,0.08)"):"rgba(255,255,255,0.03)",border:"1px solid "+(checked?(rule.personal?"#f59e0b44":"#00d4aa44"):"#ffffff11"),borderRadius:"12px",padding:"14px 16px",cursor:"pointer",transition:"all 0.2s",display:"flex",gap:"12px",alignItems:"flex-start"}}>
      <div style={{width:"22px",height:"22px",borderRadius:"6px",border:"2px solid "+(checked?color:"#555"),background:checked?color:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:"2px",transition:"all 0.2s"}}>
...
function DiarioTab({ entries, setEntries }) {
  ...
  function persist(u) { saveDiario(u); setEntries(u); }
  ...
  function deleteEntry(d) { const u={...entries}; delete u[d]; persist(u); if(expanded===d) setExpanded(null); }
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:"6px",marginBottom:"16px",flexWrap:"wrap"}}>
        {allMonths.map(m=>(
          <button key={m} onClick={()=>setSelMonth(m)} style={{padding:"5px 13px",borderRadius:"20px",cursor:"pointer",fontWeight:"600",fontSize:"11.5px",border:"none",background:selMonth===m?"#00d4aa":"rgba(255,255,255,0.05)",color:selMonth===m?"#000":"#666"}}>
            {formatMonthLabel(m)}
          </button>
        ))}
      </div>
        ...
        <>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
            {monthEntries.map(([ds,entry])=>{
              const total=(entry.b3||0)+(entry.forex||0), isToday=ds===today, isOpen=expanded===ds, hasDetail=entry.motivo||entry.nota;
              return (
                <div key={ds} style={{borderRadius:"10px",overflow:"hidden",border:"1px solid "+(isToday?"#00d4aa18":"#ffffff07"),background:isToday?"rgba(0,212,170,0.03)":"rgba(255,255,255,0.02)"}}>
                  <div style={{display:"flex",alignItems:"center",gap:"10px",padding:"11px 14px",cursor:hasDetail?"pointer":"default"}} onClick={()=>hasDetail&&setExpanded(isOpen?null:ds)}>
...
// ─── ABA REGRAS (calendario incluso) ─────────────────────────
function RegrasTab({ rules, market, checked, toggle, allChecked, bancaRealB3, bancaRealForex, compliance, setCompliance }) {
  const today   = todayKey();
  const now     = new Date();
  ...
  function toggleDay(k) {
    if (k>today) return;
    const next = compliance[k]===true ? false : compliance[k]===false ? undefined : true;
    ...
    <div>
      ...
      <div style={{marginTop:"14px",marginBottom:"28px",padding:"20px 24px",borderRadius:"12px",background:"linear-gradient(135deg,rgba(0,212,170,0.07),rgba(0,153,255,0.05))",border:"1px solid rgba(0,212,170,0.2)",textAlign:"center"}}>
        <p style={{margin:0,color:"#f0f0f0",fontWeight:"700",fontSize:"17px",letterSpacing:"0.5px",lineHeight:"1.5"}}>
          O QUE GERA RESULTADO E COMPORTAMENTO,<br/>NAO TECNICA.
        </p>
      </div>
      ...
      <div style={{borderTop:"1px solid #1a1a1a",paddingTop:"24px"}}>
        ...
        <div style={{marginBottom:"20px"}}>
          <div onClick={()=>toggleDay(today)} style={{background:todayComplied?"rgba(0,212,170,0.15)":"rgba(255,255,255,0.05)",border:"1px solid "+(todayComplied?"#00d4aa44":"#333"),borderRadius:"10px",padding:"12px 20px",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:"10px",transition:"all 0.2s"}}>
          ...
          {todayComplied && <span style={{marginLeft:"12px",color:"#00d4aa66",fontSize:"12px"}}>Registrado para {formatDateFull(today)}</span>}
        </div>
...
function BancaTab({ bancaRealB3, bancaRealForex, bancaInicialB3, bancaInicialForex, entries, setConfig }) {
  ...
  const riscoB3    = +(bancaRealB3*0.01).toFixed(2);
  const stopB3     = +(bancaRealB3*0.03).toFixed(2);
  const metaB3     = +(bancaRealB3*0.02).toFixed(2);
  const riscoFx    = +(bancaRealForex*0.01).toFixed(2);
  const stopFx     = +(bancaRealForex*0.03).toFixed(2);
  const metaFx     = +(bancaRealForex*0.02).toFixed(2);
      ...
      {editando && (
        <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid #f59e0b33",borderRadius:"14px",padding:"18px",margin:"14px 0 20px"}}>
          ...
          <div style={{display:"flex",gap:"10px",alignItems:"center"}}>
            <button onClick={salvar} style={{background:"linear-gradient(135deg,#f59e0b,#d97706)",color:"#000",border:"none",borderRadius:"8px",padding:"10px 22px",fontWeight:"700",fontSize:"13px",cursor:"pointer"}}>
...
function CrescimentoTab({ entries, bancaRealB3, bancaRealForex, bancaInicialB3, bancaInicialForex }) {
  ...
  const iB        = viewMarket==="b3" ? bancaInicialB3 : bancaInicialForex;
  ...
  const realRows = mesesComDados.map(mk=>{
    ...
    return {mk, label:formatMonthLabel(mk), bancaB3:parseFloat(accB3.toFixed(2)), bancaForex:parseFloat(accFx.toFixed(2))};
  });
  ...
  return (
    <div>
      <div style={{display:"flex",gap:"8px",marginBottom:"16px"}}>
        {["b3","forex"].map(m=>(
          <button key={m} onClick={()=>setViewMarket(m)} style={{padding:"7px 18px",borderRadius:"8px",cursor:"pointer",fontWeight:"600",fontSize:"13px",border:"none",background:viewMarket===m?(m==="b3"?"#00d4aa":"#f59e0b"):"rgba(255,255,255,0.05)",color:viewMarket===m?"#000":"#777"}}>
...
// ─── ABA PATRIMONIO ───────────────────────────────────────────
function PatrimonioTab({ entries, bancaRealB3, bancaRealForex, bancaInicialB3, bancaInicialForex }) {
  const [market,setMarket]   = useState("b3");
  const [tooltip,setTooltip] = useState(null);
  const iB      = market==="b3" ? bancaInicialB3 : bancaInicialForex;
  ...
  let accReal = iB;
  const realPoints = [
    {d:"Inicio",val:iB,label:"Inicio"},
    ...sortedDays.map(([d,e])=>{
      accReal += market==="b3" ? (e.b3||0) : (e.forex||0);
      return {d, val:parseFloat(accReal.toFixed(2)), label:formatDateFull(d)};
    })
  ];
  const proj       = gerarProjecao(bancaAt,12);
  const projPoints = [{val:bancaAt,label:"Hoje"},...proj.map((p,i)=>({val:p.metaFinal,label:"Mes "+(i+1)}))];
  ...
  return (
    <div>
      <div style={{display:"flex",gap:"8px",marginBottom:"16px"}}>
        {["b3","forex"].map(m=>(
          <button key={m} onClick={()=>setMarket(m)} style={{padding:"7px 18px",borderRadius:"8px",cursor:"pointer",fontWeight:"600",fontSize:"13px",border:"none",background:market===m?(m==="b3"?"#00d4aa":"#f59e0b"):"rgba(255,255,255,0.05)",color:market===m?"#000":"#777"}}>
            {m==="b3"?"🇧🇷 Mini Indice":"🌍 Forex"}
          </button>
        ))}
      </div>
      <div style={{padding:"10px 14px",borderRadius:"8px",background:"rgba(0,212,170,0.05)",border:"1px solid #00d4aa18",marginBottom:"16px",display:"flex",gap:"8px",alignItems:"center"}}>
        <span>🔄</span>
        <p style={{margin:0,color:"#00d4aa88",fontSize:"11.5px"}}>Curva projetada parte da <strong style={{color:"#00d4aa"}}>banca real atual</strong> ({cur} {bancaAt.toLocaleString("pt-BR",{minimumFractionDigits:2})}). Atualiza a cada registro.</p>
      </div>
...
// ─── ABA PARCIAIS ─────────────────────────────────────────────
function ParciaisTab({ bancaRealB3, bancaRealForex }) {
  const precisaForex = bancaRealForex >= 500;
  const guide = [
    {
      mercado:"Mini Indice (B3)", banca:"R$ "+bancaRealB3.toLocaleString("pt-BR",{minimumFractionDigits:2}),
      regra:"NAO faca parcial em alvos pequenos",
      quando:"Apenas se o mercado andar 2x o stop (R$ "+(bancaRealB3*0.02).toFixed(2)+") a seu favor",
      como:"Feche 50% na regiao de 2R. Mova stop para breakeven. Deixe o restante correr.",
      cor:"#00d4aa"
    },
    {
      mercado:"Forex", banca:"$ "+bancaRealForex.toLocaleString("pt-BR",{minimumFractionDigits:2}),
      regra:precisaForex?"Banca acima de $500 — parciais liberadas":"Com banca abaixo de $500, evite parciais",
      quando:precisaForex?"Feche parcial quando andar 2x o stop ($ "+(bancaRealForex*0.02).toFixed(2)+") a seu favor":"Parcial fratura o lote minimo e aumenta custo relativo",
      como:precisaForex?"Feche 50% em 2R. Mova stop para breakeven. Deixe correr.":"Leve 100% ao alvo. Revise essa regra quando a banca atingir $500.",
      cor:"#f59e0b"
    },
  ];
  return (
      ...
      {guide.map((p,i)=>(
        <div key={i} style={{border:"1px solid "+p.cor+"33",borderRadius:"14px",padding:"20px",marginBottom:"16px",background:"linear-gradient(135deg,"+p.cor+"08,transparent)"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"14px"}}>
            <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
              <div style={{width:"8px",height:"8px",borderRadius:"50%",background:p.cor}}/>
...
export default function App() {
  ...
  useEffect(()=>{
    setEntries(loadDiario());
    setCompliance(loadCompliance());
    setConfig(loadConfig());
    const onStorage = (e)=>{
      if (e.key==="diario-v3"    && e.newValue) { try{setEntries(JSON.parse(e.newValue));}catch(_){} }
      if (e.key==="compliance-v1"&& e.newValue) { try{setCompliance(JSON.parse(e.newValue));}catch(_){} }
      if (e.key==="config-v1"   && e.newValue) { try{setConfig(JSON.parse(e.newValue));}catch(_){} }
    };
    ...
    <div style={{minHeight:"100vh",background:"#0a0a0f",color:"#f0f0f0",fontFamily:"sans-serif",padding:"32px 20px"}}>
      ...
      <div style={{maxWidth:"800px",margin:"0 auto 28px"}}>
        ...
        <div style={{display:"flex",gap:"8px"}}>
          {["b3","forex"].map(m=>(
            <button key={m} onClick={()=>setMarket(m)} style={{padding:"8px 20px",borderRadius:"8px",cursor:"pointer",fontWeight:"600",fontSize:"13px",transition:"all 0.2s",background:market===m?(m==="b3"?"#00d4aa":"#f59e0b"):"rgba(255,255,255,0.05)",color:market===m?"#000":"#888",border:"1px solid "+(market===m?"transparent":"#333")}}>
              {m==="b3"?"🇧🇷 Mini Indice":"🌍 Forex"}
            </button>
          ))}
        </div>
      ...
      <div style={{maxWidth:"800px",margin:"0 auto 24px",display:"flex",gap:"4px",background:"rgba(255,255,255,0.03)",borderRadius:"12px",padding:"4px"}}>
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,padding:"9px 2px",borderRadius:"9px",cursor:"pointer",background:tab===t.id?"rgba(255,255,255,0.08)":"transparent",color:tab===t.id?"#f0f0f0":"#999",border:"none",fontSize:"14px",fontWeight:"600",transition:"all 0.2s"}}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div style={{maxWidth:"800px",margin:"0 auto"}}>
        ...
        {tab==="regras" && (
          <RegrasTab rules={rules} market={market} checked={checked} toggle={toggle} allChecked={allChecked} bancaRealB3={bancaRealB3} bancaRealForex={bancaRealForex} compliance={compliance} setCompliance={setCompliance}/>
        )}
        ...
        {tab==="parcial" && (
          <ParciaisTab bancaRealB3={bancaRealB3} bancaRealForex={bancaRealForex}/>
        )}

        {tab==="banca" && (
          <BancaTab bancaRealB3={bancaRealB3} bancaRealForex={bancaRealForex} bancaInicialB3={bancaInicialB3} bancaInicialForex={bancaInicialForex} entries={entries} setConfig={setConfig}/>
        )}
        ...
        {tab==="patrimonio" && (
          <div>
            ...
            <PatrimonioTab entries={entries} bancaRealB3={bancaRealB3} bancaRealForex={bancaRealForex} bancaInicialB3={bancaInicialB3} bancaInicialForex={bancaInicialForex}/>
          </div>
        )}
