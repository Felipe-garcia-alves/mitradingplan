...
export default function Login() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setErro(msgs[e.code] || "Erro: "+e.code);
    }
    setLoading(false);
  }

  const inp = { width:"100%", background:"rgba(255,255,255,0.04)", border:"1px solid #2a2a3a", borderRadius:"10px", padding:"13px 14px", color:"#f0f0f0", fontSize:"14px", outline:"none", boxSizing:"border-box", fontFamily:"Inter,sans-serif" };
        ...
        <div style={{background:"#0d0d14",border:"1px solid #1a1a2e",borderRadius:"16px",padding:"32px"}}>
          <div style={{display:"flex",background:"rgba(255,255,255,0.03)",borderRadius:"10px",padding:"3px",marginBottom:"24px"}}>
            {["login","cadastro"].map(m=>(
              <button key={m} onClick={()=>{setModo(m);setErro("");}} style={{flex:1,padding:"9px",borderRadius:"8px",border:"none",cursor:"pointer",fontWeight:"600",fontSize:"13px",transition:"all 0.2s",background:modo===m?"rgba(255,255,255,0.07)":"transparent",color:modo===m?"#f0f0f0":"#666",fontFamily:"Inter,sans-serif"}}>
          ...
          <p style={{textAlign:"center",color:"#888",fontSize:"12px",marginTop:"20px",lineHeight:"1.6"}}>
